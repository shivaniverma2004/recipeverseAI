...
export default function EditRecipeClient({ recipe }: { recipe: Recipe }) {
  ...
  const [coverPreview, setCoverPreview] = useState<string | null>(recipe.cover_image_url);
  const [title, setTitle] = useState(recipe.title);
  const [description, setDescription] = useState(recipe.description ?? "");
  const [ingredients, setIngredients] = useState<string[]>(recipe.ingredients);
  const [ingredientInput, setIngredientInput] = useState("");
  const [steps, setSteps] = useState<string[]>(recipe.preparation_steps.length ? recipe.preparation_steps : [""]);
  ...
  const [selectedCuisines, setSelectedCuisines] = useState<string[]>(
    Array.isArray(recipe.cuisine) ? recipe.cuisine : recipe.cuisine ? [recipe.cuisine] : []
  );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F9FBFF]">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between h-14 px-4 bg-[#F7F3E3] border-b border-[#E5E9F2]">
        ...
        <span className="text-base font-bold text-[#192A56]">Edit Recipe</span>
        <div className="w-8" />
      </div>

      <form onSubmit={handlePublish} className="flex-1 pb-24 px-6 lg:px-10 py-6 w-full max-w-2xl mx-auto">
